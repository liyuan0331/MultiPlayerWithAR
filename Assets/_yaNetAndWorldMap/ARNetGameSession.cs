...
public class ARNetGameSession : NetworkBehaviour
{
	...
	{
        planedPice = GameObject.Find("planedPice").transform;
        print(planedPice);
	...
	IEnumerator RunGame()
	{
        ...
        gameState = ARNetGameState.Gaming;
        while (true) {
            yield return new WaitForEndOfFrame();
        }
        // Reset game
    ...
    public void UpdateTargetPose(Vector3 pos, Quaternion rot) {
        //print("UpdateTargetPose");
        if (isServer&&gameState != ARNetGameState.Connecting) {
            RpcPlaceTarget(pos,rot);
        }
    }
    ...
    // Client RPCs
    [ClientRpc]
    public void RpcPlaceTarget(Vector3 pos,Quaternion rot) {
        if (!isServer) {
            planedPice.gameObject.SetActive(true);
            planedPice.transform.position = pos;
            planedPice.transform.rotation = rot;
        }
    }
